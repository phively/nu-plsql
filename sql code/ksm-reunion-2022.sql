...
,REUNION_17_COMMITTEE AS (
  SELECT DISTINCT
   ID_NUMBER
  FROM COMMITTEE
  WHERE COMMITTEE_CODE = '227' AND COMMITTEE_STATUS_CODE = 'F'
  AND START_DT = '20160901'
)
...
--- Create Subquery for Road to Reunion 2020 + 2021 Attendees

,REUNION_2021_PARTICIPANTS AS (select p.id_number
from rpt_pbh634.v_nu_event_participants_fast p
where p.event_id = '24751')

--- Create a Subquery for Road to Reunion 2020 + 2021 Registrants.
--- Also Account for Reg Date 

,REUNION_2021_REGISTRANTS AS (
...
where EP_REGISTRATION.EVENT_ID = '24751')
...
---Participants on April 30 + May 1st 
...
SELECT DISTINCT
  ...
  ,CASE WHEN REUNION2.CONTACT_ID_NUMBER IS NOT NULL THEN 'Y' END AS REGISTERED_APR30_MAY1_WEEKEND
  ,CASE WHEN P2.ID_NUMBER IS NOT NULL THEN 'Y' END AS ATTENDED_APR30_MAY1_WEEKEND
  ...
  ,CASE WHEN RC17.ID_NUMBER IS NOT NULL THEN 'Y' ELSE '' END AS "REUNION_2017_COMMITTEE"
  ...
  ,CASE WHEN R21.ID_NUMBER IS NOT NULL THEN 'Y' END AS "ATTENDED_REUNION_2021"
  ,CASE WHEN RR21.CONTACT_ID_NUMBER IS NOT NULL THEN 'Y' END AS "REGISTERED_2021_REUNION"
  ,RR21.RESPONSE_DATE AS REUNION_2021_REG_DATE
  ,CASE WHEN R20.ID_NUMBER IS NOT NULL THEN 'Y' END AS "REGISTERED FOR 2020 REUNION"
       ---- 2015 + 2016 Reunion Attendance
  ,CASE WHEN RP17.ID_NUMBER IS NOT NULL THEN 'Y' END AS "ATTENDED_REUNION_2017"
...
LEFT JOIN REUNION_2021_PARTICIPANTS R21
ON E.ID_NUMBER = R21.ID_NUMBER
LEFT JOIN REUNION_2021_REGISTRANTS RR21
ON E.ID_NUMBER = RR21.CONTACT_ID_NUMBER
...
LEFT JOIN REUNION_17_COMMITTEE RC17
 ON RC17.ID_NUMBER = E.ID_NUMBER
LEFT JOIN REUNION_2017_PARTICIPANTS RP17
 ON RP17.ID_NUMBER = E.ID_NUMBER
LEFT JOIN reunion2
     ON reunion2.CONTACT_ID_NUMBER = E.ID_NUMBER 
LEFT JOIN p2
        ON p2.ID_number = E.ID_number
